<?php
    require_once "./fetch-completed.php";
?>
...
<body>
    ...
    <div class="section2">
        ...
        <form action="admin-completed.php" method="GET">
    ...
    <?php if(count($result['res']) !== 0):?>
        ...
        <div class="paginate">
            <?php if($result['check']>1): ?>
                <a href="admin-completed.php?num=<?= $result['check']-1 ?>" class="diff">Previous</a>
            <?php else: ?>
                <div class="previous diff">Previous</div>
            <?php endif; ?>
            <div class="numbers">
                <?php for($i = 1;$i<=$result['num'];$i++): ?>
                    <a href="admin-completed.php?num=<?= $i; ?>" class="active <?= $i; ?>"><?= $i; ?></a>
                <?php endfor; ?>
            </div>
            <?php if($result['check'] < $result['num']):?>
            <a href="admin-completed.php?num=<?= $result['check']+1 ?>" class="diff">Next</a>
            <?php else: ?>
        ...
        <div class="paginate">
            <?php if($result['check']>1): ?>
                <a href="admin-completed.php?num=<?= $result['check']-1 ?>&search=<?= htmlentities($_GET['search']) ?>" class="diff">Previous</a>
            <?php else: ?>
                <div class="previous diff">Previous</div>
            <?php endif; ?>
            <div class="numbers">
                <?php for($i = 1;$i<=$result['num'];$i++): ?>
                    <a href="admin-completed.php?num=<?= $i; ?>&search=<?= htmlentities($_GET['search']) ?>" class="active <?= $i; ?>"><?= $i; ?></a>
                <?php endfor; ?>
            </div>
            <?php if($result['check'] < $result['num']):?>
            <a href="admin-completed.php?num=<?= $result['check']+1 ?>&search=<?= htmlentities($_GET['search']) ?>" class="diff">Next</a>
            <?php else: ?>
                <div class="next diff">Next</div>
            <?php endif;?>
        </div>
